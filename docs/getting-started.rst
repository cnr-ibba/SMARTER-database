...
There's also the need to upload data even for the custom chips, in order to have
a more precise picture of all the variants. Data need also to be integrated with
additional information like breeds and their code. All those steps are managed
...
an external MongoDB instance. See :ref:`Configure environment variables` for more
information.

Installation and configuration
------------------------------

Here are the instructions to correctly install dependencies and set up a functional
project.
...
`SNPchimp`_, `Ensembl`_ or `EVA`_. Moreover, you need also information from
...
or ``background`` folder accordingly if data were produced in the context of SMARTER
or are available outside this project. External source files, like manifests, database
...
you need to ask to developer and to the WP4 coordinator to have access to this data.
...
To upload all the external information on *variants* in the database. Next with:

.. code-block:: bash

  make data

You will import samples information into database and you will generate two plink files,
for sheep and goat respectively, with all the genotype in the same format and
assembly version. Last step in data generation is made available with:

.. code-block:: bash

  make publish

which will pack your genotype files in order to be shared with the partners using
...
to import into database or you can export a collection outside *SMARTER-database*.
Here are describe how to dump and restore a *SMARTER-database* instance:
